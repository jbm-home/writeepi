...
export class Scheduler {
    ...
    Schedule(name: string, action: () => void, interval: number, first: number = 0) {
        this.log.info(`Scheduling new task '${name}' with start at ${first / 1000}s and interval ${interval / 1000}s`);
        setInterval(() => {
