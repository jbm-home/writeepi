...
SessionController.get('/users', Authorized.ADMIN, async (req, res): Promise<any> => {
    return res
        .status(200)
        .json(await service.allUsers(req));
});

SessionController.get('/token', Authorized.USER, async (req, res): Promise<any> => {
    return res
        .status(200)
        .json(await service.simpleToken(req));
});

SessionController.put('/users/:uuid', Authorized.ADMIN, async (req, res): Promise<any> => {
    return res
        .status(200)
        .json(await service.updateUser(req));
});

SessionController.put('/users/phone/:uuid', Authorized.ADMIN, async (req, res): Promise<any> => {
    return res
        .status(200)
        .json(await service.updateUserPhone(req));
});

SessionController.post('/login', Authorized.GUESTONLY, async (req, res): Promise<any> => {
    return res
        .status(200)
        .json(await service.login(req));
});

SessionController.post('/register', Authorized.GUESTONLY, async (req, res): Promise<any> => {
    return res
        .status(200)
