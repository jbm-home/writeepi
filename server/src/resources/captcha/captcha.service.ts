...
export class CaptchaService {
    ...
    async build(req: any) {
        ...
        const image = new Jimp({ width: this.width, height: this.height, color: 'white' });
    ...
    randomHash(len: number) {
        return Array.from(
            crypto.getRandomValues(new Uint8Array(Math.ceil(len / 2))), (b) => ("0" + (b & 0xFF).toString(16)).slice(-2)).join("");
    }
}