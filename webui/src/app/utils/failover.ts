export class Failover {
    ...
    static getBestFailover() {
        let foundFailovers: any[] = [];
        Failover.FAILOVERS.forEach(f => foundFailovers.push({ name: f, data: localStorage.getItem(f) }));
        ...
        return foundFailovers.sort((a, b) => JSON.parse(a.data ?? defaultData).time - JSON.parse(b.data ?? defaultData).time)[0].name;
    }

    static hasAvailableFailover() {
        return Failover.FAILOVERS.find(f => localStorage.getItem(f) !== null) !== undefined;
    }
        ...
        Failover.FAILOVERS.forEach(f => {
            ...
            if (current !== null) {
                ...
                if (p.data !== undefined) {
                    ...
                    foundFailovers.push({ id: inc++, time: p.time, title: p.data?.title, data: p.data });
                }
            }
        });
